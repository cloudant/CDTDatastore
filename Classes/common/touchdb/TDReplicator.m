...
- (void)setLastSequence:(NSString*)lastSequence
{
    if (!$equal(lastSequence, _lastSequence)) {
        LogWarn(REPLICATION_LOG_CONTEXT, @"%@: Setting lastSequence to %@ (from %@)", self,
...
- (void)stopped
{
    ...
    LogInfo(REPLICATION_LOG_CONTEXT, @"Replication: %@ took %.3f sec; error=%@", self,
...
{
    LogInfo(REPLICATION_LOG_CONTEXT, @"%@: Reachability state = %@ (%02X)", self, host,
...
- (void)checkSession
{
    ...
    [self sendAsyncRequest:@"GET"
              ...
              onCompletion:^(id result, NSError* error) {
                  if (error) {
                      LogInfo(REPLICATION_LOG_CONTEXT, @"%@: Session check failed: %@", self,
                              error);
                      self.error = error;
                  } else {
                      ...
                      if (username) {
                          LogInfo(REPLICATION_LOG_CONTEXT, @"%@: Active session, logged in as '%@'",
                                  self, username);
                          [self fetchRemoteCheckpointDoc];
                      } else {
                          [self login];
...
- (void)login
{
    ...
    LogInfo(REPLICATION_LOG_CONTEXT, @"%@: Logging in with %@ at %@ ...", self, _authorizer.class,
            loginPath);
    [self asyncTaskStarted];
...
- (void)stopRemoteRequests
{
    ...
    LogInfo(REPLICATION_LOG_CONTEXT, @"Stopping %u remote requests",
        ...
        sendAsyncRequest:@"GET"
            ...
            onCompletion:^(id response, NSError* error) {
                ...
                if (error && error.code != kTDStatusNotFound) {
                    LogInfo(REPLICATION_LOG_CONTEXT, @"%@: Error fetching last sequence: %@", self,
                    ...
                    if ($equal(remoteLastSequence, localLastSequence)) {
                        ...
                        LogInfo(REPLICATION_LOG_CONTEXT, @"%@: Replicating from lastSequence=%@",
                                self, _lastSequence);
                    } else {
                        LogInfo(
...
- (void)saveLastSequence
{
    ...
    [self sendAsyncRequest:@"PUT"
              ...
              onCompletion:^(id response, NSError* error) {
                  ...
                  if (error) {
                      LogWarn(REPLICATION_LOG_CONTEXT, @"%@: Unable to save remote checkpoint: %@",
                              self, error);
