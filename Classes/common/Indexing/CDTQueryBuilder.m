                                     ...
                                     error: (NSError* __autoreleasing *) error
{
    ...
    for (NSString* table in keySet) {
        NSString *subclause = [NSString stringWithFormat:@"%@%@ as %@", kCDTIndexTablePrefix,
                                                                table, table];
                                      ...
                                      error: (NSError* __autoreleasing *) error
{
    ...
    while ((table = [enumerator nextObject]) != nil) {
        NSString *subclause = [NSString stringWithFormat:@"%@.docid = %@.docid",
                               firstTable, table];
        [subclauses addObject:subclause];
    }
    
    // Second part of the where clause are the predicates (e.g., a.value > ?)
    NSString *sqlFragment = [self toSQLFragmentWithPredicate: predicate
                                          extractQueryValues: queryValues
                                                       error: error];
    if (sqlFragment == nil) {
                                   ...
                                   error: (NSError* __autoreleasing *) error
{
    if ([predicate isKindOfClass:([NSCompoundPredicate class])]) {
        ...
        NSDictionary *ui = @{
                             ...
                             NSLocalizedRecoverySuggestionErrorKey: @"Use NSCompound Predicate or NSComparisonPredicate",
                       ...
                       error: (NSError* __autoreleasing *) error
{
    if ([predicate isKindOfClass: [NSCompoundPredicate class]]) {
        return [self accumulateKeysIntoSet: keySet
                   fromCompoundPredicate: (NSCompoundPredicate*) predicate error: error];
    } else if ([predicate isKindOfClass: [NSComparisonPredicate class]]){
        return [self accumulateKeysIntoSet: keySet
                 fromComparisonPredicate: (NSComparisonPredicate*) predicate error: error];
    } else {
        NSDictionary *ui = @{
                             NSLocalizedDescriptionKey: @"Unsupported Predicate class",
                             NSLocalizedRecoverySuggestionErrorKey: @"Use NSCompound Predicate or NSComparisonPredicate",
...
+ (NSString*) toSQLFragmentWithKeyValuePredicate: (NSComparisonPredicate*) comparisonPredicate
    ...
    NSDictionary* keyValue = [self getKeyValueFromComparisonPredicate:comparisonPredicate
                                                                error:error];
    if (keyValue == nil) {
    ...
    switch ([comparisonPredicate predicateOperatorType]) {
        ...
        case NSGreaterThanPredicateOperatorType: operator =  @">";break;
        case NSGreaterThanOrEqualToPredicateOperatorType: operator = @">=";break;
        case NSEqualToPredicateOperatorType: operator = @"=";break;
    ...
    if ([lhs expressionType] == NSKeyPathExpressionType) {
        if ([rhs expressionType] == NSKeyPathExpressionType) {
            *error = [NSError errorWithDomain: CDTQueryBuilderErrorDomain
                                     ...
                                     userInfo:@{
            NSLocalizedDescriptionKey: @"Multiple keys specified in a NSComparisonPredicate",
            NSLocalizedRecoverySuggestionErrorKey:@"Use only one key in NSComparisonPredicate",
            NSLocalizedFailureReasonErrorKey:
                [NSString stringWithFormat:  @"%@ %@ are both keys in an NSComparisonPredicate",
                                        lhs, rhs]}];;
        } else {
            keyValue = @{@"key": [lhs keyPath],
                         @"value": [rhs expressionValueWithObject:nil context:nil]
                         };
        }
    } else if ([rhs expressionType] == NSKeyPathExpressionType) {
        keyValue = @{@"key": [rhs keyPath],
        ...
        *error = [NSError errorWithDomain: CDTQueryBuilderErrorDomain
        ...
        NSLocalizedRecoverySuggestionErrorKey: @"Use one key in NSComparisonPredicate",
        NSLocalizedFailureReasonErrorKey:
            [NSString stringWithFormat:  @"%@ %@ are not keys in an NSComparisonPredicate",
                                            lhs, rhs]}];;
    }
    return keyValue;
}
        ...
        default:
            *error = [NSError errorWithDomain: CDTQueryBuilderErrorDomain
                                     ...
                                     userInfo:@{
                                                ...
                                                NSLocalizedRecoverySuggestionErrorKey:
                                                    @"Use NSCompoundPredicate types NSAndPredicateType, NSOrPredicateType or "
                                                "NSNotPredicateType",
                                                NSLocalizedFailureReasonErrorKey:@"Unsupported NSCompoundPredicate type"}];
            success = NO;
    }
                       ...
                       error: (NSError* __autoreleasing *) error
{
    ...
    for (NSPredicate* subpredicate in [compoundPredicate subpredicates]) {
        bool success = [self accumulateKeysIntoSet: keyReferences
                                   fromPredicate: subpredicate
                                           error: error];
        if (!success) {
