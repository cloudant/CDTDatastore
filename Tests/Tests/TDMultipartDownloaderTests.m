        ...
        requestHeaders:nil
          onCompletion:^(id result, NSError* error) {
            ...
            XCTAssertTrue(attachments.count >= 1, @"attachments.count >= 1 fails in %s",
                          __PRETTY_FUNCTION__);
            XCTAssertEqual(db.attachmentStore.count, 0u,
                           @"db.attachmentStore.count is not 0u in %s", __PRETTY_FUNCTION__);
            for (NSDictionary* attachment in attachments.allValues) {
                ...
                XCTAssertTrue([writer install], @"TDBlobStoreWriter install returned NO in %s",
                              __PRETTY_FUNCTION__);
                ...
                XCTAssertEqual(blob.length, [lengthObj unsignedLongLongValue],
                               @"blob length and object length are not equal in %s",
                               __PRETTY_FUNCTION__);
                XCTAssertEqual(writer.length, blob.length,
                               @"writer length and blog length are not equal in %s",
                               __PRETTY_FUNCTION__);
            }
            XCTAssertEqual(db.attachmentStore.count, attachments.count,
